...
exports.findAll = (req, res) => {
  ...
  const TeamId = req.query.Keyword;
...
exports.findOne = (req, res) => {
  Belong.findById(req.params.id, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found Belong with id ${req.params.id}.`
        });
      } else {
        res.status(500).send({
          message: "Error retrieving Belong with id " + req.params.id
        });
      }
    } else res.send(data);
  });
    ...
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          ...
          res.status(500).send({
            message: "Error updating Belong with id " + req.params.id
          });
        }
      } else res.send(data);
    }
...
exports.delete = (req, res) => {
  Belong.remove(req.params.id, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found Belong with id ${req.params.id}.`
        ...
        res.status(500).send({
          message: "Could not delete Belong with id " + req.params.id
        });
      }
