...
exports.findOne = (req, res) => {
  Skin.findById(req.params.SkinId, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found Skin with id ${req.params.SkinId}.`
        });
      } else {
        res.status(500).send({
          message: "Error retrieving Skin with id " + req.params.SkinId
        });
      }
    } else res.send(data);
  });
...
exports.update = (req, res) => {
  ...
  Skin.updateById(
    req.params.SkinId,            //by id, 注这里 params和之前body不一样
    ...
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          ...
          res.status(500).send({
            message: "Error updating Skin with id " + req.params.SkinId
          });
        }
      } else res.send(data);
    }
...
exports.delete = (req, res) => {
  Skin.remove(req.params.SkinId, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found Skin with id ${req.params.SkinId}.`
        ...
        res.status(500).send({
          message: "Could not delete Skin with id " + req.params.SkinId
        });
      }
